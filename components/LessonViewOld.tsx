...
export default function LessonView({ koLesson, enLesson, jaLesson, category, relatedEntries }: Props) {
  ...
  const handleUpdateBlock = (stepIndex: number, blockIndex: number, newBlock: Block) => {
    // 텍스트 변경도 히스토리에 저장
    updateLessonData(lesson => ({
      ...lesson,
      steps: lesson.steps.map((step, i) => 
        i === stepIndex 
          ? { 
              ...step, 
              blocks: step.blocks.map((block, j) => 
                j === blockIndex ? newBlock : block
              ) 
            }
          : step
      )
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  onClick={handleRedo}
                  disabled={redoStack.ko.length === 0}
                  className="p-1.5 rounded-lg text-xs font-medium disabled:opacity-50 disabled:cursor-not-allowed bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors"
        ...
        {lesson.steps.map((step, i) => {
          ...
          return (
            <section
              ...
              id={`step-${step.number}`}
              className="rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-visible"
            >
              <div className="p-8 space-y-4 pl-16">
